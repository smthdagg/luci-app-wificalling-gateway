...
END {
  ...
  for(k=1;k<=nn;k++) {
    ...
    if (p=="vless") {
      ...
      else { sni=(f[7]!=""?f[7]:f[19]); if (f[16]=="tls"||sni!="") s=s ",\"tls\":" tls(sni,f[8],f[9],f[20]) }
    }
    if (p=="vmess") {
      ...
      # certificate verification has a name to check.
      sni=(f[7]!=""?f[7]:f[19])
      if (f[16]=="tls"||sni!="") s=s ",\"tls\":" tls(sni,f[8],f[9],f[20])
    }
    s=s "}"; print "    " s ","
  }
