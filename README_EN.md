...
- **Automatic DHCP static lease management**: adding/removing a device policy auto-binds/cleans the MAC-IP static lease, tolerating iOS rotating private Wi-Fi addresses (random MACs); the device policy table shows the live binding state (Bound / Not bound yet / MAC changed / Online (static IP) / Device offline) - ARP fallback keeps static-IP and DHCP-less setups reporting online.
