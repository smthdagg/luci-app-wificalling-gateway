...
- **Per-node instant test**: every node row has a "nodeTest" button — WireGuard nodes re-run a fresh handshake immediately (bypassing the cache), other protocols get a TCP probe, and the result (exit IP or failure reason) is shown as a banner.
