...
- **活动日志**：显示最近 100 条加密 IMS 活动，可自动刷新、查看记录数，并在确认后清空。
...
设备监控还记录最近 100 条加密 IMS 活动，包括时间、状态变化和包量增量。由于通话与短信均位于加密 IPsec 隧道内，插件不能识别号码、正文，也不能可靠区分一次活动究竟是通话还是短信。“清空日志”只清理本插件的活动文件，不影响系统日志、节点或设备配置。
