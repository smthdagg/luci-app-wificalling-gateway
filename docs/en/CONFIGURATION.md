...
The monitor retains the latest 100 encrypted IMS activity events with timestamps, state changes, and packet deltas. Calls and SMS are encrypted inside IPsec, so the router cannot identify numbers or content and cannot reliably distinguish a call from a text message. Clearing this history does not modify system logs, nodes, or device policies.
