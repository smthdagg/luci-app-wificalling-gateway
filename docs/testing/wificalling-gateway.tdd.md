...
- GREEN: the v1.2 runner executes 25 tests successfully, covering all five original journeys plus separate management pages, scoped log clearing, integrated node quality, Wi-Fi Calling evidence, encrypted IMS event logging, invalid protocol, duplicate IP, non-private IP, unrelated-device traffic, PassWall bypass, package format, release metadata, documentation, and LuCI regressions.
...
- Target-router validation: passed on Redmi AX6S, ImmortalWrt 24.10.6 (`aarch64_cortex-a53`) with sing-box 1.13.16. The installed package passed `sing-box check`, kept both procd instances alive, installed scoped nftables TProxy rules, routed only the selected test client, established the configured AnyTLS tunnel, and observed an ASSURED bidirectional UDP 4500 flow to an ePDG endpoint. A completed real call was used as the carrier voice acceptance check.
...
- LuCI runtime ACLs cover both `/var/run` and its OpenWrt canonical `/tmp/run` path. Status and activity reads were verified through an ordinary authenticated LuCI session.
- Settings, live status, and activity history are separate pages. Log clearing is confirmation-protected and its write ACL is limited to the plugin event file.
